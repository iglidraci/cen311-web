...
if (isset($_POST['submit'])){
    ...
    if (count($validation_messages) === 0) {
        ...
        if (insert_record('book', $data)) {
            echo '<div class="alert alert-success" role="alert">
                        Book added successfully!
                    </div>';
        } else {
            echo '<div class="alert alert-danger" role="alert">
                    Book insertion failed!</div>';
        }
        //insert into book (title, isbn, author_id, current_price, original_price, stock, published_date) 
        // values (:title, :isbn, :author_id, :current_price, :original_price, :stock, :published_date);
        // try {
        //     $reset_form = true;
        //     $sql = "insert into book (title, isbn, author_id, current_price, original_price, published_date, stock) 
        //             values (:title, :isbn, :author_id, :current_price, :original_price, :published_date, :stock)";
        //     $stmt = $pdo -> prepare($sql);
        //     $stmt -> bindParam(":title", $clean['title']);
        //     $stmt -> bindParam(":isbn", $clean['isbn']);
        //     $stmt -> bindParam(":author_id", $clean['author_id']);
        //     $stmt -> bindParam(":current_price", $clean['current_price']);
        //     $stmt -> bindParam(":original_price", $clean['original_price']);
        //     $stmt -> bindParam(":published_date", $clean['published_date']);
        //     $stmt -> bindParam(":stock", $clean['stock']);
        //     $stmt -> execute();
        //     echo '<div class="alert alert-success" role="alert">
        //                 Book added successfully!
        //             </div>';
        ...
        //     echo '<div class="alert alert-danger" role="alert">
        //             Book insertion failed! ' . $ex -> getMessage() . '</div>';
        // }
    }
}
