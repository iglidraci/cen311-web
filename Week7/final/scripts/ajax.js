...
const getSeasonsPerLeagueWithPromise = (leagueId) => {
    ...
    fetch(url)
        ...
        .then(json => {
            renderJsonToTable(json.data);
            // throw new Error("Because I say so 😂");
            const currentYear = new Date().getFullYear();
            return fetch(`${BASE_URL}/leagues/${leagueId}/standings?season=${currentYear}&sort=asc`);
        })
        .then(response => response.json())
        .then(json => setStandingTeams(json.data))
        .catch(handleError)
        .finally(() => stopLoading());
}
...
container.addEventListener('click', (event) => {
    ...
    if (event.target.classList.contains('btn-season')) {
        ...
        leagueId = id;
        getSeasonsPerLeagueWithPromise(id);
